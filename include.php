...
function keyExists($db,$key){
	$result=my_redis()->multi()->select($db)->exists($key)->exec();
	return $result[1];
}
...
function engageKey($db,$key){
	for($i=0;$i<2000;$i++){
		$result=my_redis()->multi()->select($db)->hGetAll($key)->hSetNx($key,"engaged",microtime(true))->exec();
		if(count($result[1]) <= 0 || $result[1]===false){
			return null;
		}
		if($result[2]==false){
			if(microtime(true)-$result[1]['engaged']<1){
				usleep(1000);
				continue;
			}
